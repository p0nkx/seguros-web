  ...
  return (
    <div className="max-w-5xl mx-auto px-4">
      ...
      <div className="flex justify-end mb-8">
        <button
          onClick={abrirNuevo}
          className="bg-[#001f3d] text-white px-8 py-4 rounded-2xl font-bold shadow-xl hover:bg-blue-900 transition-all cursor-pointer flex items-center gap-2 transform hover:scale-105"
        >
          <span className="text-xl">+</span> Nuevo Asegurado
        </button>
      </div>
        ...
        <div className="flex bg-white p-1 rounded-2xl shadow-lg border border-slate-100">
          {(["todos", "activos", "bajas"] as const).map((estado) => (
            <button
              ...
              className={`px-6 py-2 rounded-xl cursor-pointer text-[10px] font-bold uppercase transition-all ${filtroEstado === estado
              ...
              <div className={`transition-all duration-500 ease-in-out overflow-hidden ${abiertoId === cliente.id ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="p-6 bg-slate-50 border-t border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    ...
                    {/* BOTÓN WHATSAPP */}
                    {/* Sección de contacto en ClientesList.tsx */}
                      ...
                      {cliente.celular && (
                        <>
                          <WhatsappButton
                            ...
                            isFloating={false} // IMPORTANTE: para que no se vaya a la esquina de la pantalla
                          />
                          <span className="text-xs font-bold text-slate-500 uppercase">Contactar por WhatsApp</span>
                        </>
                      )}
                    </div>
                  </div>

                  {/* COLUMNA 2: DETALLES DE CUENTA */}
                  <div className="space-y-4">
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm space-y-4">
                      ...
                      <div className="space-y-1">
                        ...
                        {edad !== null && (
                          <div className="flex justify-between text-sm">
                            <span className="text-slate-400 italic">Edad actual:</span>
                      ...
                      <div className="pt-3 border-t border-slate-100 flex justify-between items-center">
                        <span className="text-[11px] text-slate-400 font-bold uppercase">Referencia</span>
                        <span className="bg-slate-800 text-white px-3 py-1 rounded-md font-mono text-sm font-bold shadow-sm">
                          #{cliente.id}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="md:col-span-2 border-t border-gray-200 pt-4 mt-2">
                    <button
                      ...
                      className="w-full md:w-auto px-6 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-2"
                    >
